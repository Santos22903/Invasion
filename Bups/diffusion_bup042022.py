...
def initializeGlobals():
    ...
    ## k_on, k_off read in from config, but will use intrinsic, so these not global
    if len(sys.argv)>2:
    ...
    global diff_coef_scaled, diff_delt
    global k_a_intr, k_d_intr
    ...
    ### binding parameters
    # intrinsic association constant. Obtained from (S2) in Fange/Berg/Sjoberg/Elf
    # delx/2 here is the "reaction radius" - when closer to node than half distance b/n nodes
    # units: delx is in um, diff_coef is um^2/s, k_on has molar: mole/dm^3
    # so, need dm^3 -> um^3 to "align"
    dm_to_um = 1e5
    k_a_intr = 4*math.pi*(delx/2)*diff_coef*k_on*pow(dm_to_um,3)/(4*math.pi*(delx/2)*diff_coef - k_on*pow(dm_to_um,3))
    # intrinsic dissociation constant
    # using that ratios of intrinsic and macroscopic constants are the same
    k_d_intr = k_a_intr*k_off/k_on

    #
    print("4*math.pi*(delx/2)*diff_coef ",4*math.pi*(delx/2)*diff_coef)
    print("k_on*pow(dm_to_um,3) ",k_on*pow(dm_to_um,3))
    print("4*math.pi*(delx/2)*diff_coef-k_on*pow(dm_to_um,3) ",4*math.pi*(delx/2)*diff_coef-k_on*pow(dm_to_um,3))
    print("k_a_intr ",k_a_intr)
    print("k_on ",k_on)
    #
...
def doStepping(benefarray,
    ...
    for stepN in range(Nsteps_max):
        ...
        if DEBUG and (stepN in steps2plot): # preparing to plot FG coords for some steps if debugging
    ...
    if DEBUG: # plotting prepared GF coords if debugging
...
def main():
    ...
    print("diff_coef, delx in main ", round(diff_coef,3), delx)
    print("Calculated global constants")
    print("diff_coef_scaled, diff_delt in main ",round(diff_coef_scaled,3), round(diff_delt,3))
