...
def bindingStep(benefarray, gfcoords, useMultiproc=True, useMap=True):
    ...
    if useMultiproc:
        pool = mp.Pool(cores)
        resarray = np.array(pool.starmap(GillespieFn,zip(benefarray_1d,coordmat_1d,rands0,rands1)),dtype="int")
        pool.close()
    elif useMap:
        resarray = np.array(list(map(GillespieFn,benefarray_1d,coordmat_1d,rands0,rands1)),dtype="int")
    else:
        resarray = np.array([GillespieFn(x,y,z,a) for x,y,z,a in zip(benefarray_1d,coordmat_1d,rands0,rands1)],dtype="int")
...
def GillespieFn(benefBound,benefFree,rand0,rand1):
    """
...
def doStepping(benefarray,
    ...
    for stepN in range(Nsteps_max):
        ...
        #print(stepN)
        (benefarray, gfcoords) = bindingStep(benefarray, gfcoords, useMultiproc=True, useMap=False)
