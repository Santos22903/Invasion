...
def initializeGlobals():
    ...
    global n_runs, findContrib, cores2use
...
def printGlobalVars():
    ...
    print("n_runs, findContrib, cores2use ",n_runs,findContrib,cores2use)
...
def strategyUpdate(cellarray,benefitarray,local_rng):
    """
    ...
    ## choose node to test
    test_node = local_rng.choice(poss_coords,size=2)
    #print("test_node ",test_node)
    
    ## choose one of its neighbors as a "role model"
    model_axis = local_rng.choice(poss_axis)
    model_dir  = local_rng.choice(poss_dirs)
...
def one_run(local_seed):
    global n_cycles, invasion_done_perc, useCost, cost
    invasion_ctr = 0
    perishing_ctr= 0
    ...
    for cycleN in range(n_cycles):
        ...
        ## if using cost, reduce benefits of "2"s by the cost
        if useCost:
        ...
        ## birth - death game
        cellArray = strategyUpdate(cellArray,benefitArray,local_rng)
    ...
    return invasion_ctr, perishing_ctr
