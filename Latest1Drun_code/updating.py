...
def wrightFisherSpatial3D(cellarray,fitnesses,rng_arg,expanding,newL=None):
    if expanding:
        assert newL is not None
    ...
    if expanding:
        ...
        side = len(cellarray)
    new_cellarray = np.ones((side,side,side))
    N = np.count_nonzero(final_array==2)
    if N>=cellNumber:
        print("Number of 2's exceeded lattice size, returning lattice full of 2's")
        new_cellarray = np.full((side,side,side),2)
        return new_cellarray
    side_of_subarray = math.ceil(N**(1./3.))
    array_of_2_1d = np.ones(side_of_subarray*side_of_subarray*side_of_subarray)
    array_of_2_1d[0:N] = 2
    array_of_2_3d = array_of_2_1d.reshape(side_of_subarray,side_of_subarray,side_of_subarray)
    loFilled = (side-side_of_subarray)//2
    if loFilled<0:
        loFilled = loFilled + 1
    hiFilled = loFilled + side_of_subarray
    new_cellarray[loFilled:hiFilled,loFilled:hiFilled,loFilled:hiFilled] = array_of_2_3d
    return new_cellarray

def wrightFisherSpatial1D(cellarray,fitnesses,rng_arg,expanding,newL=None):
    if expanding:
        assert newL is not None
    ...
    if expanding:
        ...
        side = len(cellarray)
    new_cellarray = np.ones((side))
    N = np.count_nonzero(final_array==2)
    if N>=cellNumber:
        print("Number of 2's exceeded lattice size, returning lattice full of 2's")
        new_cellarray = np.full((side),2)
        return new_cellarray
    side_of_subarray = N
    array_of_2_1d = np.ones(side_of_subarray)
    array_of_2_1d[0:N] = 2
    loFilled = (side-side_of_subarray)//2
    if loFilled<0:
        loFilled = loFilled + 1
    hiFilled = loFilled + side_of_subarray
    new_cellarray[loFilled:hiFilled] = array_of_2_1d
    return new_cellarray
